...
void parse_tree::print (std::ostream& out, const pnode& pos, size_t level) {
...
void parse_tree::add_tree(const terminal& to_add, const parse_tree& tree, const pnode& pos) {
    if (! pos) {
        return;
    }
    if (pos->_value->class_name() == "terminal" && pos->_children.empty()) {
        pos->_children.push_back(std::make_shared<node>());
...
void parse_tree::copy_tree(pnode& to, pnode from) {
...
bool parse_tree::add_token(const terminal& to_add, const token& v, pnode& pos) {
    if (! pos) {
        return false;
    }
    if (pos->_value->class_name() == "terminal" && pos->_children.empty()) {
...
bool parse_tree::add_product(const terminal& to_add, type_product product, pnode& pos) {
    if (! pos) {
        return false;
    }
    if (pos->_value->class_name() == "terminal" && pos->_children.empty()) {
...
void parse_tree::push_product_item(std::vector<std::shared_ptr<node>>& children, type_product product) {
    if (product == FUNCTION) {
