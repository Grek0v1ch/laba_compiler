...
void semantic::feel_descr(std::shared_ptr<tree_node>& curr_descr) {
    ...
    int cnt_var = feel_var_list(curr_descr->_children[1]);
    // Собираем Descr
...
int semantic::feel_var_list(std::shared_ptr<tree_node>& var_list) {
    // Число переменных
    int cnt_var = 0;
    // Если дальше есть еще var list, то сначала разбираем его
    if (var_list->_children.size() == 3) {
        cnt_var = feel_var_list(var_list->_children[2]);
...
void semantic::feel_op(std::shared_ptr<tree_node>& curr_op) {
    ...
    feel_expr(curr_op->_children[2]);
    curr_op->_rpn = curr_op->_children[2]->_rpn + " " + curr_op->_rpn;
}

void semantic::feel_expr(std::shared_ptr<tree_node>& curr_expr) {
    feel_simple_expr(curr_expr->_children[0]);
    ...
    feel_expr(curr_expr->_children[2]);
    curr_expr->_rpn += " " + oper->text();
    curr_expr->_rpn = curr_expr->_children[2]->_rpn + " " + curr_expr->_rpn;
}

void semantic::feel_simple_expr(std::shared_ptr<tree_node>& curr_simple_expr) {
    if (curr_simple_expr->_children.size() == 3) {
        feel_expr(curr_simple_expr->_children[1]);
